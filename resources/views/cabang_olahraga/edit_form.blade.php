...
@section('content')
    <div class="bg-white shadow-lg border rounded-xl p-5">
        <form id="form" method="POST" enctype="multipart/form-data" action="{{route('cabang_olahraga.update', $cabor->id)}}">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-y-2">
                <div>
                    ...
                    <div>
                        <input class="border-2 w-1/2 rounded-lg px-2 py-1" name="nama_cabang_olahraga" value="{{$cabor->nama}}">
                    </div>
                </div>
            </div>
            <div class="mt-10">
